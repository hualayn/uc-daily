...
**An Android app for recording daily life with ulcerative colitis (UC)**

Every day, log what you eat, your bowel movements, your medications, and how you feel — so you can look back and answer *"what did I eat before the flare started?"*, and easily share everything with your doctor at follow-up visits.

</div>

> 📄 **简体中文版**：[README-cn.md](./README-cn.md)
>
> 📦 **APK download** (latest release, auto-built by GitHub Actions after each `v*` tag): [uc-daily.apk](https://github.com/hualayn/uc-daily/releases/latest/download/uc-daily.apk)
...
<table>
  <tr>
    <td align="center" width="50%"><img src="docs/screenshots/home.png" width="250" alt="Home"></td>
    <td align="center" width="50%"><img src="docs/screenshots/tolerance.png" width="250" alt="Tolerance"></td>
  </tr>
  <tr>
    <td align="center"><b>Home</b><br>Calendar (week / full month) · daily stats · today's records</td>
    <td align="center"><b>Tolerance</b><br>Food tolerance: tolerable / try / intolerant</td>
  </tr>
  <tr>
    <td align="center" width="50%"><img src="docs/screenshots/daily-management.png" width="250" alt="Daily Management"></td>
    <td align="center" width="50%"><img src="docs/screenshots/profile.png" width="250" alt="Profile"></td>
  </tr>
  <tr>
    <td align="center"><b>Daily Management</b><br>Evidence-based handbook · self-assessment</td>
    <td align="center"><b>Profile</b><br>Statistics · export / restore · settings</td>
  </tr>
</table>

## What this app is for

Built for people living with ulcerative colitis (UC). It helps you:

- **Find your triggers.** Log every meal (photos + food tags). When a flare starts, look back at the days before it and see what you had eaten. The **Tolerance** page keeps a running list of what you can eat, what to try cautiously, and what to avoid — each food shows how many meal records tagged it, so problem foods stand out.
- **Monitor disease activity.** Every bowel-movement record is automatically scored with a simplified, patient-self-report UCDAI and shown as a colored dot on the calendar: 🟢 remission · 🟡 mild · 🟠 moderate · 🔴 severe.
- **Stay on top of medication.** Set the daily reminder count and times. A missed dose lights the home bell in red and fires a system notification.
- **Show your doctor real data.** Export any date range as TXT or CSV and hand it over at your next visit.
- **Keep everything private.** All data lives on your device (local Room database + app-private photo storage). No accounts, no cloud, no network required.

> ⚠️ The activity score and the Daily Management handbook are for self-monitoring and reference only — they do **not** replace a doctor's diagnosis.

## Pages

Four bottom tabs + a center raised **"+"** button (opens the quick-add panel from any tab: 🍚 Meal · 💩 Bowel movement · 💊 Medication · 📝 Note).

### 🏠 Home

- **Welcome card**: avatar + time-of-day greeting + a rotating daily encouragement banner (the slogan list is editable under *Profile → Settings → Home Slogans*); a medication reminder bell in the top-right corner (shows a red dot when a reminder time has passed but the dose is not recorded yet; tap it to jump back to today and open the add-medication panel; a red system notification is posted at the same time)
...
- Daily management handbook (accordion cards, one open at a time, *Diet* open by default): **Diet** / **Lifestyle** / **Medication & Medical Care** / **Psychological Adjustment** / **Self-Assessment & Symptom Reference**
...
- Evidence-based reference information — for self-reference only, not a substitute for a doctor's diagnosis
...
- **Theme**: light / dark / follow system (switch under "Profile → Settings → Theme")
- **Multilingual**: follows the system language by default; you can manually switch between 12 options under "Profile → Settings → Language" (follow system / 简体中文 / English / 日本語 / 한국어 / Français / Deutsch / Italiano / Español / Português / Русский / العربية). The UI, the medication-reminder notification and its channel description follow the selected language; Arabic automatically enables RTL layout
- **In-app updates**: integrated Google Play Core (app-update 2.x) Flexible in-app updates — a silent check at startup, plus a manual check under "Profile → Settings → App Update"; when a new version is found: "Update now" → background download → "Restart now" to apply
...
- **Delivery via Google Play Core**: language resources ship inside the App Bundle (AAB) and Google Play delivers only the languages relevant to each user ("deliver languages relevant to the user" is enabled in Play Console), so users never download extra language packs. New versions (with new languages / features) are pushed through Google Play and can be installed in-app via Play Core (silent check at startup + manual check under "Profile → Settings → App Update" → background download → restart to apply). Note: in-app updates only work for builds installed from Google Play; sideloaded builds degrade silently.
- **Publishing tips**: when publishing the AAB in Play Console, tick "deliver only languages relevant to the user" to enable per-language delivery; keep `android:supportsRtl="true"` in the Manifest so Arabic renders right-to-left correctly.
...
├── UcDailyApp.kt         # Application: applies the selected locale (default: follow system)
├── AppLocale.kt          # Multilingual: language options, persistence, attachBaseContext wrapping
├── MainActivity.kt       # Entry: bottom tab layout, camera/gallery launchers, global panel layer, in-app update UI
├── MedReminder.kt        # Med reminders: post/cancel system notification + alarm scheduling + alarm/boot receivers (works in background)
├── MedReminderService.kt # Med-reminder service (coroutine scope for reminder sync)
├── play/
│   └── AppUpdate.kt      # Google Play Core in-app update (Flexible): check / download / restart
├── ui/
│   ├── MealLogViewModel.kt      # ViewModel: state management (single MealUiState), drafts, data operations
│   ├── HomeScreen.kt            # Home tab: welcome card (med bell), calendar (week / full month), daily stats, today's records
│   ├── ToleranceScreen.kt       # Tolerance tab: food tolerance (tap to delete / drag to reorder / drag across sections to change state)
│   ├── DailyManagementScreen.kt # Daily Management tab: management handbook (accordion cards) + self-assessment score
│   ├── ProfileScreen.kt         # Profile tab: avatar / nickname + menu (stats / med settings / export / restore / settings)
...
│   ├── DesignSystem.kt          # Design system: palette, cards, buttons, badges, section headers, top bar
│   └── Theme.kt                 # Blue color theme (light / dark)
├── data/
│   ├── MealRecord.kt / MealRecordDao.kt     # Meal record entity + meal-type enum (food tags JSON codec)
│   ├── DailySymptom.kt / DailySymptomDao.kt # Bowel/symptom entity + activity score (simplified UCDAI)
│   ├── MedRecord.kt / MedRecordDao.kt       # Medication record entity
│   ├── DailyNote.kt / DailyNoteDao.kt       # Daily note entity (unique per date)
│   ├── FoodTag.kt / FoodTagDao.kt           # Food tag entity + tolerance enum (tolerable / try / intolerant) + sort key
│   ├── RestoreImporter.kt                   # CSV restore: parses this app's own export format
│   └── AppDatabase.kt                       # Room database config (v1, fresh schema, no legacy migrations)
└── util/
    └── PhotoCompressor.kt      # Photo compression (JPEG < 300 KB before storage)
```
