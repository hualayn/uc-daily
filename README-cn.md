...
- **服药提醒**：在"我的 → 服药设置"设置提醒次数 / 时间；判定口径为"已到点的提醒时间数 > 当天服药记录数"即视为未服药。未服药时：首页铃铛亮红点（点击直达添加服药）+ 系统通知（状态栏常驻红色图标 / 应用图标角标，文案"您还有 N 次未服药，请尽快服药！"）。到点触发由系统普通闹钟（AlarmManager，`setAndAllowWhileIdle`，无需任何闹钟权限）驱动——应用处于后台 / 被系统杀掉时到点同样提醒，重启后自动重注册；仅需通知权限（Android 13+ 启动时运行时申请）
...
│   ├── MedSettingsScreen.kt     # 服药设置页：提醒次数、提醒时间
...
- 服药提醒：通知走系统通知渠道 `med_reminder`（常驻、可着色）；到点触发用系统普通闹钟（AlarmManager，`setAndAllowWhileIdle`，最多 6 个提醒时间各占一个槽位，无需精确闹钟权限），应用启动 / 修改提醒时间 / 开机完成时重注册
